...
var activeButton = 0;
let responseData = {};
// How to embed a table with externdal fields from an api (23 lines). ChatGPT, 4 Sep. version, OpenAI, 11 Sep. 2023, chat.openai.com/chat.
function showLessData() {
    document.getElementById("singleResponse").innerHTML = "";
        ...
        if (totalResult > 0) {
            ...
            for (let i = 0; i < minItems; i++) {
                ...
                if (shippingServiceCost && Array.isArray(shippingServiceCost) && shippingServiceCost.length > 0) {
                    ...
                    tag1 += '<div class="card" onclick="ClickData(this)" data-index="' + i + '">';
                    tag1 += '<table id="data-id" >\n';
                    tag1 += '<tr id="dataR">\n';
...
// How to create a function from redirecting (3 lines). ChatGPT, 4 Sep. version, OpenAI, 11 Sep. 2023, chat.openai.com/chat.
function stopPropagation(event) {
    event.stopPropagation();
}
// How to embed a table with externdal fields from an api (26 lines). ChatGPT, 4 Sep. version, OpenAI, 11 Sep. 2023, chat.openai.com/chat.
function showMoreData(){
    ...
    for (let i = 0; i < minItems; i++) {
        ...
        tag1 += '<div class="card" data-index="' + i + '" onclick="ClickData(this)">';
        tag1 += '<table id="data-id" >\n';
        tag1 += '<tr id="dataR">\n';
        tag1 += '<td id="galleryUrl" rowspan="4" style="min-height: 100px;">\n';
        tag1 += '<div class="image-container">';
...
function ClickData(clickedElement){
    ...
    let itemId = globalResult['data']['searchResult'][0]['item'][dataIndexNumber]['itemId'][0];
    let queryParams = [];
    if (itemId) {
    ...
    fetch(requestURL).then(response => response.json()).then(data => {
        ...
        let defaultImageURL = "https://csci571.com/hw/hw6/images/ebay_default.jpg"; 
        let pictureUrl = data['Item']['PictureURL'][0] ? data['Item']['PictureURL'][0] : defaultImageURL;
        let ebayLink = data['Item']['ViewItemURLForNaturalSearch'] ? data['Item']['ViewItemURLForNaturalSearch'] : "#";
        let title = data['Item']['Title'];
        let price = data['Item']['CurrentPrice']['Value'];
        let location = data['Item']['Location']+","+data['Item']['PostalCode'];
        let seller = data['Item']['Seller']['UserID'];
        let itemSpecifics = data['Item']['ItemSpecifics']['NameValueList'];
        let returnPolicy = data['Item']['ReturnPolicy']? (data['Item']['ReturnPolicy']['ReturnsAccepted']?data['Item']['ReturnPolicy']['ReturnsAccepted']:"") : "";
        let returnsWithin = data['Item']['ReturnPolicy']?(data['Item']['ReturnPolicy']['ReturnsWithin']?data['Item']['ReturnPolicy']['ReturnsWithin']:"") : "";
        let values = "";
        if (!returnPolicy && !returnsWithin) {
          values = "";
        } else {    
          returnPolicy = returnPolicy || "";
          returnsWithin = returnsWithin ? " within " + returnsWithin : "";
          let returnPolicyUS = returnPolicy + returnsWithin;
          values = returnPolicyUS;
        }
        let tag1 = "";
        ...
        tag1 += '<td class="table-data">'+values+'</td>'
        tag1 += '</tr>\n';
